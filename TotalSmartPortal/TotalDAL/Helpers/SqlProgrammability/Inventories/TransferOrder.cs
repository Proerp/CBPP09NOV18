...
namespace TotalDAL.Helpers.SqlProgrammability.Inventories
{
    public class TransferOrder
    {
        ...
        private void GetTransferOrderPendingBlendingInstructions()
        {
            ...
            queryString = queryString + "       DECLARE         @BlendingInstructionDetails TABLE (CommodityID int NOT NULL, CustomerName nvarchar(100) NULL, BlendingInstructionCode nvarchar(50) NULL, BlendingInstructionSpecs nvarchar(800) NULL, QuantityRemains decimal(18, 2) NOT NULL, QuantityTransferOrders decimal(18, 2) NOT NULL, QuantityAvailableL2 decimal(18, 2) NOT NULL) " + "\r\n";


            queryString = queryString + "       INSERT INTO     @BlendingInstructionDetails (CommodityID, CustomerName, BlendingInstructionCode, BlendingInstructionSpecs, QuantityRemains, QuantityTransferOrders, QuantityAvailableL2) " + "\r\n";
            queryString = queryString + "       SELECT          BlendingInstructionDetails.CommodityID, NULL, BlendingInstructions.Code, Commodities.Name, ROUND(BlendingInstructionDetails.Quantity - BlendingInstructionDetails.QuantityIssued, " + (int)GlobalEnums.rndQuantity + ") AS QuantityRemains, 0 AS QuantityTransferOrders, 0 AS QuantityAvailableL2 FROM BlendingInstructionDetails INNER JOIN BlendingInstructions ON BlendingInstructionDetails.BlendingInstructionID = BlendingInstructions.BlendingInstructionID INNER JOIN Commodities ON BlendingInstructions.CommodityID = Commodities.CommodityID WHERE BlendingInstructionDetails.InActive = 0 AND BlendingInstructionDetails.InActivePartial = 0 AND ROUND(BlendingInstructionDetails.Quantity - BlendingInstructionDetails.QuantityIssued, " + (int)GlobalEnums.rndQuantity + ") > 0 " + "\r\n"; //Approved = 1 AND 

            queryString = queryString + "       INSERT INTO     @BlendingInstructionDetails (CommodityID, CustomerName, BlendingInstructionCode, BlendingInstructionSpecs, QuantityRemains, QuantityTransferOrders, QuantityAvailableL2) " + "\r\n";
            queryString = queryString + "       SELECT          CommodityID, NULL, NULL, NULL, 0 AS QuantityRemains, ROUND(Quantity - QuantityIssued, " + (int)GlobalEnums.rndQuantity + ") AS QuantityTransferOrders, 0 AS QuantityAvailableL2 FROM TransferOrderDetails WHERE CommodityID IN (SELECT CommodityID FROM @BlendingInstructionDetails) AND LocationIssuedID = 1 AND LocationReceiptID = 2 AND TransferOrderID <> @TransferOrderID AND InActive = 0 AND InActivePartial = 0 AND ROUND(Quantity - QuantityIssued, " + (int)GlobalEnums.rndQuantity + ") > 0 " + "\r\n"; //Approved = 1 AND 

            queryString = queryString + "       INSERT INTO     @BlendingInstructionDetails (CommodityID, CustomerName, BlendingInstructionCode, BlendingInstructionSpecs, QuantityRemains, QuantityTransferOrders, QuantityAvailableL2) " + "\r\n";
            queryString = queryString + "       SELECT          GoodsReceiptDetails.CommodityID, NULL, NULL, NULL, 0 AS QuantityRemains, 0 AS QuantityTransferOrders, ROUND(GoodsReceiptDetails.Quantity - GoodsReceiptDetails.QuantityIssued, " + (int)GlobalEnums.rndQuantity + ") AS QuantityRemains FROM GoodsReceiptDetails INNER JOIN Warehouses ON GoodsReceiptDetails.WarehouseID = Warehouses.WarehouseID WHERE GoodsReceiptDetails.CommodityID IN (SELECT CommodityID FROM @BlendingInstructionDetails) AND Warehouses.LocationID = 2 AND ROUND(GoodsReceiptDetails.Quantity - GoodsReceiptDetails.QuantityIssued, " + (int)GlobalEnums.rndQuantity + ") > 0 " + "\r\n";
            ...
            queryString = queryString + "                       SUM(BlendingInstructionDetails.QuantityRemains) AS QuantityRemains, SUM(BlendingInstructionDetails.QuantityTransferOrders) AS QuantityTransferOrders, SUM(BlendingInstructionDetails.QuantityAvailableL2) AS QuantityAvailableL2, CAST(0 AS bit) AS IsSelected " + "\r\n";
        ...
        private void GetTransferOrderPendingWorkOrders()
        {
            ...
            queryString = queryString + "       DECLARE         @WorkOrderDetails TABLE (CommodityID int NOT NULL, CustomerName nvarchar(100) NULL, FirmOrderCode nvarchar(50) NULL, FirmOrderSpecs nvarchar(800) NULL, QuantityRemains decimal(18, 2) NOT NULL, QuantityTransferOrders decimal(18, 2) NOT NULL, QuantityAvailableL2 decimal(18, 2) NOT NULL) " + "\r\n";
            ...
            queryString = queryString + "                       SUM(WorkOrderDetails.QuantityRemains) AS QuantityRemains, SUM(WorkOrderDetails.QuantityTransferOrders) AS QuantityTransferOrders, SUM(WorkOrderDetails.QuantityAvailableL2) AS QuantityAvailableL2, CAST(0 AS bit) AS IsSelected " + "\r\n";
