...
namespace TotalDAL.Helpers.SqlProgrammability.Inventories
{
    public class GoodsReceipt
    {
        ...
        private void GetGoodsReceiptIndexes()
        {
            ...
            queryString = queryString + "       SELECT      GoodsReceipts.GoodsReceiptID, CAST(GoodsReceipts.EntryDate AS DATE) AS EntryDate, GoodsReceipts.Reference, Locations.Code AS LocationCode, Workshifts.Name AS WorkshiftName, Users.FirstName AS UserFirstName, Users.LastName AS UserLastName, Customers.Code AS CustomerCode, Customers.Name AS CustomerName, ISNULL(PlannedOrders.Reference, '') +  ISNULL(' (' + PlannedOrders.Code + ')', '') + ISNULL(WarehouseIssues.Code, '') + IIF(GoodsReceipts.GoodsReceiptTypeID = 7, N'Thu hồi màng đã cấp', '') AS GoodsReceiptTypeCaption, GoodsReceipts.Caption, GoodsReceipts.Description, " + "\r\n";
            ...
            queryString = queryString + "                   INNER JOIN Locations ON (GoodsReceipts.GoodsReceiptTypeID = 6 OR GoodsReceipts.GoodsReceiptTypeID = 7 OR GoodsReceipts.GoodsReceiptTypeID = 8 OR GoodsReceipts.GoodsReceiptTypeID = 18 " + (GlobalEnums.CBPP ? "" : "OR GoodsReceipts.GoodsReceiptTypeID = 9") + ") AND GoodsReceipts.NMVNTaskID = @NMVNTaskID AND GoodsReceipts.EntryDate >= @FromDate AND GoodsReceipts.EntryDate <= @ToDate AND GoodsReceipts.OrganizationalUnitID IN (SELECT AccessControls.OrganizationalUnitID FROM AccessControls INNER JOIN AspNetUsers ON AccessControls.UserID = AspNetUsers.UserID WHERE AspNetUsers.Id = @AspUserID AND AccessControls.NMVNTaskID = @NMVNTaskID AND AccessControls.AccessLevel > 0) AND Locations.LocationID = GoodsReceipts.LocationID " + "\r\n";
        ...
        private void GetGoodsReceiptPendingPurchasings()
        {
            ...
            queryString = queryString + "       DECLARE         @GoodsArrivalCount int              DECLARE @PurchaseOrderCodes varchar(3999)           DECLARE @CustomerOfficialNames varchar(555) " + "\r\n";
            queryString = queryString + "       DECLARE         @GoodsArrivals TABLE (PurchaseOrderCode nvarchar(60), CustomerOfficialName nvarchar(150) NOT NULL)" + "\r\n";

            queryString = queryString + "       INSERT INTO     @GoodsArrivals (PurchaseOrderCode, CustomerOfficialName) " + "\r\n";
            queryString = queryString + "       SELECT          ISNULL(GoodsArrivals.PurchaseOrderCodes, GoodsArrivals.Code) AS PurchaseOrderCode, Customers.OfficialName AS CustomerOfficialName " + "\r\n";
            ...
            queryString = queryString + "       BEGIN " + "\r\n";
            queryString = queryString + "           SELECT      @PurchaseOrderCodes = LEFT(STUFF((SELECT ',' + PurchaseOrderCode FROM @GoodsArrivals FOR XML PATH('')) ,1,1,''), 100) " + "\r\n";
            queryString = queryString + "           SELECT      @CustomerOfficialNames = LEFT(STUFF((SELECT ',' + CustomerOfficialName FROM (SELECT DISTINCT CustomerOfficialName FROM @GoodsArrivals) DistinctCustomerOfficialNames FOR XML PATH('')) ,1,1,''), 100) " + "\r\n";
            queryString = queryString + "       END " + "\r\n";
