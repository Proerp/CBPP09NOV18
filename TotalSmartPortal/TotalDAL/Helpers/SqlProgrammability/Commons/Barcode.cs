...
namespace TotalDAL.Helpers.SqlProgrammability.Commons
{
    public class Barcode
    {
        ...
        private void GetBarcodeJournals()
        {
            ...
            queryString = queryString + "               INSERT INTO     @BarcodeJournals (EntryID, EntryDate, Reference, ControllerName, Barcode, Quantity, QuantityRemains, TareWeight, Approved, LabApproved, LabHold, LabInActive, BinLocationCode, BlendingInstructionCode, ShortName, UserFirstName, Description) " + "\r\n";
            queryString = queryString + "               SELECT          ISNULL(GoodsReceiptDetails.WarehouseTransferID, GoodsReceiptDetails.GoodsReceiptID) AS EntryID, GoodsReceiptDetails.EntryDate, ISNULL(WarehouseTransfers.Reference, GoodsReceiptDetails.Reference) AS Reference, 'Inventories/' + IIF(GoodsReceiptDetails.WarehouseTransferID IS NULL, 'MaterialReceipts', 'MaterialTransfers') AS ControllerName, GoodsReceiptDetails.Barcode, GoodsReceiptDetails.Quantity, GoodsReceiptDetails.Quantity - GoodsReceiptDetails.QuantityIssued AS QuantityRemains, GoodsReceiptDetails.TareWeight, GoodsReceiptDetails.Approved, Labs.Approved AS LabApproved, Labs.Hold AS LabHold, Labs.InActive AS LabInActive, BinLocations.Code AS BinLocationCode, NULL AS BlendingInstructionCode, NULL AS ShortName, Users.FirstName AS UserFirstName, IIF(GoodsReceiptDetails.WarehouseTransferID IS NULL, N'Nhập kho', N'VCNB') AS Description " + "\r\n";
            ...
            queryString = queryString + "               INSERT INTO     @BarcodeJournals (EntryID, EntryDate, Reference, ControllerName, Barcode, Quantity, QuantityRemains, TareWeight, Approved, LabApproved, LabHold, LabInActive, BinLocationCode, BlendingInstructionCode, ShortName, UserFirstName, Description) " + "\r\n";
            queryString = queryString + "               SELECT          WarehouseAdjustmentDetails.WarehouseAdjustmentID AS EntryID, WarehouseAdjustmentDetails.EntryDate, WarehouseAdjustmentDetails.Reference, 'Inventories/OtherMaterialIssues' AS ControllerName, GoodsReceiptDetails.Barcode, -WarehouseAdjustmentDetails.Quantity, NULL AS QuantityRemains, NULL AS TareWeight, WarehouseAdjustmentDetails.Approved, CAST(1 AS bit) AS LabApproved, CAST(0 AS bit) AS LabHold, CAST(0 AS bit) AS LabInActive, BinLocations.Code AS BinLocationCode, NULL AS BlendingInstructionCode, NULL AS ShortName, Users.LastName AS UserFirstName, N'Xuất khác' AS Description " + "\r\n";
            ...
            queryString = queryString + "               INSERT INTO     @BarcodeJournals (EntryID, EntryDate, Reference, ControllerName, Barcode, Quantity, QuantityRemains, TareWeight, Approved, LabApproved, LabHold, LabInActive, BinLocationCode, BlendingInstructionCode, ShortName, UserFirstName, Description) " + "\r\n";
            queryString = queryString + "               SELECT          PackageIssueDetails.PackageIssueID AS EntryID, PackageIssueDetails.EntryDate, PackageIssueDetails.Reference, 'Inventories/PackageIssues' AS ControllerName, PackageIssueDetails.Barcode, PackageIssueDetails.Quantity, NULL AS QuantityRemains, NULL AS TareWeight, PackageIssueDetails.Approved, CAST(1 AS bit) AS LabApproved, CAST(0 AS bit) AS LabHold, CAST(0 AS bit) AS LabInActive, NULL AS BinLocationCode, BlendingInstructions.Code AS BlendingInstructionCode, Commodities.Code AS ShortName, Users.FirstName AS UserFirstName, N'Đổ bồn' AS Description " + "\r\n";
            ...
            queryString = queryString + "       SELECT EntryID, EntryDate, Reference, ControllerName, Barcode, IIF(Quantity = 0, NULL, Quantity) AS Quantity, IIF(ROUND(QuantityRemains, " + (int)GlobalEnums.rndQuantity + ") = 0, NULL, QuantityRemains) AS QuantityRemains, TareWeight, Approved, LabApproved, LabHold, LabInActive, BinLocationCode, BlendingInstructionCode, ShortName, UserFirstName, Description FROM @BarcodeJournals " + "\r\n";
            queryString = queryString + "    END " + "\r\n";
