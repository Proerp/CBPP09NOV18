...
namespace TotalDAL.Helpers.SqlProgrammability.Reports
{
    public class InventoryReports
    {
        ...
        private void WarehouseCards()
        {
            ...
            queryString = queryString + "                   SELECT      " + (int)GlobalEnums.WarehouseAdjustmentTypeID.IssueINV + " AS JournalTypeID, Customers.CustomerID, Customers.Code AS CustomerCode, Customers.Name AS CustomerName, N'' AS Specs, N'' AS Specification, NULL AS FirmOrderCode, 'Inventories/GoodsIssues' AS ControllerName, GoodsIssuePackages.GoodsIssueID AS EntityID, N'HH TẠI KHO' AS GroupName, CONVERT(VARCHAR, @LocalFromDate, 103) + ' -> ' + CONVERT(VARCHAR, @LocalToDate, 103) AS SubGroupName, GoodsReceiptDetails.GoodsReceiptDetailID, GoodsIssuePackages.EntryDate, GoodsReceiptDetails.CommodityID, GoodsIssuePackages.Reference, GoodsReceiptDetails.Reference AS Referral, GoodsReceiptDetails.Code AS ReceiptCode, GoodsReceiptDetails.Barcode, GoodsReceiptDetails.WarehouseID, NULL AS Description, 0 AS QuantityDebit, GoodsIssuePackages.Quantity AS QuantityCredit " + "\r\n";
