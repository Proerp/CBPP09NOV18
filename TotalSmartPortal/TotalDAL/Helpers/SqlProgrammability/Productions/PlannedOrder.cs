...
namespace TotalDAL.Helpers.SqlProgrammability.Productions
{
    public class PlannedOrder
    {
        ...
        private void GetPlannedOrderIndexes()
        {
            ...
            queryString = queryString + "                                               FirmOrderID int NULL, FirmOrderDetailID int NULL, SerialID int NULL, HasProductionOrders int NULL, CommodityCode nvarchar(50) NULL, CommodityName nvarchar(200) NULL, Approved bit NOT NULL, InActive bit NOT NULL, InActivePartial bit NOT NULL, VoidTypeName nvarchar(50) NULL, QuantityRequested decimal(18, 2) NULL, QuantityOnhand decimal(18, 2) NULL, Quantity decimal(18, 2) NULL, MoldWeight decimal(18, 2) NULL, PiecePerPack int NULL, " + "\r\n";
            queryString = queryString + "                                               ItemCode nvarchar(50) NULL, ItemWeight decimal(18, 2) NULL, ItemEntryDate datetime NULL, ItemQuantity decimal(18, 2) NULL, ItemQuantitySemifinished decimal(18, 2) NULL, ItemQuantityFailure decimal(18, 2) NULL, ItemQuantityReceipted decimal(18, 2) NULL, ItemQuantityLoss decimal(18, 2) NULL, " + "\r\n";
            ...
            queryString = queryString + "                   FirmOrderID, FirmOrderDetailID, SerialID, HasProductionOrders, CommodityCode, CommodityName, Approved, InActive, InActivePartial, VoidTypeName, QuantityRequested, QuantityOnhand, Quantity, IIF(QuantitySemifinished - QuantityShortage - QuantityFailure + QuantityExcess = 0, NULL, QuantitySemifinished - QuantityShortage - QuantityFailure + QuantityExcess) AS QuantityProduced, MoldWeight, PiecePerPack, " + "\r\n";
            queryString = queryString + "                   ItemCode, ItemWeight, ItemEntryDate, IIF(SerialID = 0, ItemQuantity, NULL) AS ItemQuantity, IIF(SerialID = 0 AND ItemQuantitySemifinished <> 0, ItemQuantitySemifinished, NULL) AS ItemQuantitySemifinished, IIF(SerialID = 0, ItemQuantityFailure, NULL) AS ItemQuantityFailure, IIF(SerialID = 0, ItemQuantityReceipted, NULL) AS ItemQuantityReceipted, IIF(SerialID = 0, ItemQuantityLoss, NULL) AS ItemQuantityLoss, IIF(SerialID = 0, ItemQuantity - ItemQuantityReceipted, NULL) AS ItemQuantityNet," + "\r\n";
        ...
        private string GetPlannedOrderIndexSQL(int dateOptionID, int filterOptionID)
        {
            ...
            queryString = queryString + "                                         FirmOrderID, FirmOrderDetailID, SerialID, HasProductionOrders, CommodityCode, CommodityName, Approved, InActive, InActivePartial, VoidTypeName, QuantityRequested, QuantityOnhand, Quantity, MoldWeight, PiecePerPack, " + "\r\n";
            queryString = queryString + "                                         ItemCode, ItemWeight, ItemEntryDate, ItemQuantity, ItemQuantitySemifinished, ItemQuantityFailure, ItemQuantityReceipted, ItemQuantityLoss, " + "\r\n";
            ...
            { //THE SAME INSERT QUERY
                ...
                queryString = queryString + "   SELECT      PlannedOrders.PlannedOrderID, CAST(" + this.SQLDateOption(dateOptionID) + " AS DATE) AS EntryDate, PlannedOrders.EntryDate AS PlannedOrderEntryDate, PlannedOrders.Reference, PlannedOrders.Code, PlannedOrders.VoucherDate, PlannedOrders.DeliveryDate, Customers.Code AS CustomerCode, Customers.Name AS CustomerName, PlannedOrders.Description, " + "\r\n";
                queryString = queryString + "               NULL AS FirmOrderID, NULL AS FirmOrderDetailID, NULL AS SerialID, NULL AS HasProductionOrders, Commodities.Code AS CommodityCode, Commodities.Name AS CommodityName, PlannedOrders.Approved, PlannedOrders.InActive, 0 AS InActivePartial, VoidTypes.Name AS VoidTypeName, PlannedOrderDetails.QuantityRequested, PlannedOrderDetails.QuantityOnhand, PlannedOrderDetails.Quantity, Molds.Weight AS MoldWeight, PlannedOrderDetails.PiecePerPack, " + "\r\n";
                queryString = queryString + "               BomItems.Code AS ItemCode, BomItems.Weight AS ItemWeight, NULL AS ItemEntryDate, NULL AS ItemQuantity, NULL AS ItemQuantitySemifinished, NULL AS ItemQuantityFailure, NULL AS ItemQuantityReceipted, NULL AS ItemQuantityLoss, " + "\r\n";
        ...
        private void PlannedOrderToggleApproved()
        {
            ...
            queryString = queryString + "                       INSERT INTO     FirmOrderMaterials (FirmOrderID, PlannedOrderID, NMVNTaskID, EntryDate, LocationID, CustomerID, BomID, BlockUnit, BlockQuantity, MaterialID, Quantity, QuantityIssued, VoidTypeID, Approved, InActive, InActivePartial, InActivePartialDate) " + "\r\n";
            queryString = queryString + "                       SELECT          FirmOrderDetails.FirmOrderID, FirmOrderDetails.PlannedOrderID, FirmOrderDetails.NMVNTaskID, FirmOrderDetails.EntryDate, FirmOrderDetails.LocationID, FirmOrderDetails.CustomerID, FirmOrderDetails.BomID, FirmOrderDetails.BlockUnit, FirmOrderDetails.BlockQuantity, BomDetails.MaterialID, ROUND(SUM(((FirmOrderDetails.Quantity * BomDetails.BlockUnit)/ 100) * BomDetails.BlockQuantity/ BomDetails.LayerQuantity), " + (int)GlobalEnums.rndQuantity + ") AS Quantity, 0 AS QuantityIssued, FirmOrderDetails.VoidTypeID, FirmOrderDetails.Approved, FirmOrderDetails.InActive, FirmOrderDetails.InActivePartial, FirmOrderDetails.InActivePartialDate " + "\r\n";
            ...
            queryString = queryString + "                       GROUP BY        FirmOrderDetails.FirmOrderID, FirmOrderDetails.PlannedOrderID, FirmOrderDetails.NMVNTaskID, FirmOrderDetails.EntryDate, FirmOrderDetails.LocationID, FirmOrderDetails.CustomerID, FirmOrderDetails.BomID, FirmOrderDetails.BlockUnit, FirmOrderDetails.BlockQuantity, BomDetails.MaterialID, FirmOrderDetails.VoidTypeID, FirmOrderDetails.Approved, FirmOrderDetails.InActive, FirmOrderDetails.InActivePartial, FirmOrderDetails.InActivePartialDate; " + "\r\n";
