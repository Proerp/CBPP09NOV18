...
namespace TotalDAL.Helpers.SqlProgrammability.Productions
{
    public class SemifinishedItem
    {
        ...
        private void GetSemifinishedItemIndexes()
        {
            ...
            queryString = queryString + "       SELECT      SemifinishedItems.SemifinishedItemID, CAST(SemifinishedItems.EntryDate AS DATE) AS EntryDate, SemifinishedItems.Reference, Locations.Code AS LocationCode, Customers.Code AS CustomerCode, Customers.Name AS CustomerName, ProductionLines.Code AS ProductionLineCode, SemifinishedItems.WorkshiftID, Workshifts.EntryDate AS WorkshiftEntryDate, Workshifts.Code AS WorkshiftCode, FirmOrders.Reference AS FirmOrdersReference, FirmOrders.Code AS FirmOrdersCode, FirmOrders.Specification, SemifinishedItems.StartSequenceNo, SemifinishedItems.StopSequenceNo, SemifinishedItems.Description, SemifinishedItems.TotalQuantity, SemifinishedItems.Approved " + "\r\n";
        ...
        private void GetSemifinishedItemViewDetails()
        {
            ...
            queryString = queryString + "                       FirmOrderDetails.MoldQuantity, ROUND(FirmOrderDetails.Quantity - (FirmOrderDetails.QuantitySemifinished - FirmOrderDetails.QuantityShortage - FirmOrderDetails.QuantityFailure + FirmOrderDetails.QuantityExcess) + ISNULL(SemifinishedItemDetails.Quantity, 0), " + (int)GlobalEnums.rndQuantity + ") AS QuantityRemains, ISNULL(SemifinishedItemDetails.Quantity, 0) AS Quantity, SemifinishedItemDetails.Remarks " + "\r\n";
        ...
        private void GetSemifinishedItemPendingMaterialIssues()
        {
            ...
            queryString = queryString + "       SELECT          MaterialIssues.MaterialIssueID, MaterialIssues.PlannedOrderID, MaterialIssues.FirmOrderID, FirmOrders.EntryDate AS FirmOrderEntryDate, FirmOrders.Reference AS FirmOrderReference, FirmOrders.Code AS FirmOrderCode, FirmOrders.Specification AS FirmOrderSpecification, Boms.Code AS BomCode, " + "\r\n";
            ...
            queryString = queryString + "                       INNER JOIN FirmOrders ON FirmOrders.InActive = 0 AND MaterialIssues.MaterialIssueID IN (SELECT MaterialIssueID FROM MaterialIssueDetails WHERE LocationID = @LocationID AND Approved = 1 AND ROUND(Quantity - QuantitySemifinished - QuantityFailure - QuantityReceipted - QuantityLoss, " + (int)GlobalEnums.rndQuantity + ") > 0) AND MaterialIssues.FirmOrderID = FirmOrders.FirmOrderID " + "\r\n";            
        ...
        private void SemifinishedItemSaveRelative()
        {
            ...
            queryString = queryString + "       DECLARE         @SemifinishedItemDetails TABLE (FirmOrderDetailID int NOT NULL PRIMARY KEY, PlannedOrderDetailID int NOT NULL, Quantity decimal(18, 2) NOT NULL)" + "\r\n";
            queryString = queryString + "       INSERT INTO     @SemifinishedItemDetails (FirmOrderDetailID, PlannedOrderDetailID, Quantity) SELECT FirmOrderDetailID, PlannedOrderDetailID, SUM(Quantity) AS Quantity FROM SemifinishedItemDetails WHERE SemifinishedItemID = @EntityID GROUP BY FirmOrderDetailID, PlannedOrderDetailID " + "\r\n";

            queryString = queryString + "       UPDATE          FirmOrderDetails " + "\r\n";
            queryString = queryString + "       SET             FirmOrderDetails.QuantitySemifinished = ROUND(FirmOrderDetails.QuantitySemifinished + SemifinishedItemDetails.Quantity * @SaveRelativeOption, " + (int)GlobalEnums.rndQuantity + ") " + "\r\n";
            queryString = queryString + "       FROM            FirmOrderDetails " + "\r\n";
            ...
            queryString = queryString + "       SET             MaterialIssueDetails.QuantitySemifinished = ROUND(MaterialIssueDetails.QuantitySemifinished + SemifinishedItems.FoilWeights * @SaveRelativeOption, " + (int)GlobalEnums.rndQuantity + "), MaterialIssueDetails.QuantityFailure = ROUND(MaterialIssueDetails.QuantityFailure + SemifinishedItems.FailureWeights * @SaveRelativeOption, " + (int)GlobalEnums.rndQuantity + ") " + "\r\n";
            queryString = queryString + "       FROM            MaterialIssueDetails " + "\r\n";
            queryString = queryString + "                       INNER JOIN SemifinishedItems ON SemifinishedItems.SemifinishedItemID = @EntityID AND MaterialIssueDetails.MaterialIssueDetailID = SemifinishedItems.MaterialIssueDetailID " + "\r\n";
