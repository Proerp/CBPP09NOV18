...
namespace TotalDAL.Helpers.SqlProgrammability.Productions
{
    public class WorkOrder
    {
        ...
        private void GetWorkOrderIndexes()
        {
            ...
            queryString = queryString + "       SELECT      WorkOrders.WorkOrderID, CAST(WorkOrders.EntryDate AS DATE) AS EntryDate, WorkOrders.Reference, Locations.Code AS LocationCode, Customers.Name AS CustomerName, ISNULL(FirmOrders.Description, '') + ' ' + ISNULL(WorkOrders.Description, '') AS Description, WorkOrders.TotalQuantity, WorkOrders.Approved, " + "\r\n";
        ...
        private void GetWorkOrderViewDetails()
        {
            ...
            queryString = queryString + "                   FirmOrderMaterials.Quantity AS FirmOrderMaterialQuantity, FirmOrderMaterials.QuantityIssued AS FirmOrderMaterialQuantityIssued, GoodsReceiptDetails.QuantityAvailables, ISNULL(WorkOrderDetails.Quantity, 0) AS Quantity " + "\r\n";
